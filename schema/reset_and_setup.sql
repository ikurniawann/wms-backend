-- RESET & SETUP SCRIPT FOR SUPABASE - FIXED VERSION
-- WARNING: This will DROP ALL existing tables and data!
-- Generated: 2026-04-07 (Fixed)

-- ============================================
-- STEP 1: CLEANUP EXISTING DATA (Safe)
-- ============================================

-- Drop policies if tables exist
DO $$
BEGIN
    -- Drop policies (ignore errors if not exists)
    ...
    DROP POLICY IF EXISTS units_isolation ON master_data.units;
    DROP POLICY IF EXISTS categories_isolation ON master_data.categories;
    DROP POLICY IF EXISTS attributes_isolation ON master_data.attributes;
    DROP POLICY IF EXISTS products_isolation ON master_data.products;
    DROP POLICY IF EXISTS variants_isolation ON master_data.product_variants;
    DROP POLICY IF EXISTS stocks_isolation ON inventory.stocks;
    DROP POLICY IF EXISTS storage_bins_isolation ON inventory.storage_bins;
    DROP POLICY IF EXISTS movements_isolation ON inventory.movements;
    DROP POLICY IF EXISTS customers_isolation ON sales.customers;
    DROP POLICY IF EXISTS orders_isolation ON sales.orders;
    DROP POLICY IF EXISTS suppliers_isolation ON purchase.suppliers;
    DROP POLICY IF EXISTS po_isolation ON purchase.orders;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Some policies did not exist, continuing...';
END $$;

-- Drop views
DROP VIEW IF EXISTS inventory.v_stock_summary CASCADE;
DROP VIEW IF EXISTS sales.v_daily_sales CASCADE;
DROP VIEW IF EXISTS sales.v_product_performance CASCADE;

-- Drop tables in reverse dependency order (with IF EXISTS)
DROP TABLE IF EXISTS inventory.movement_details CASCADE;
DROP TABLE IF EXISTS inventory.movements CASCADE;
DROP TABLE IF EXISTS inventory.stock_bins CASCADE;
DROP TABLE IF EXISTS inventory.reservations CASCADE;
DROP TABLE IF EXISTS inventory.stocks CASCADE;
DROP TABLE IF EXISTS inventory.storage_bins CASCADE;

DROP TABLE IF EXISTS sales.order_items CASCADE;
DROP TABLE IF EXISTS sales.orders CASCADE;
DROP TABLE IF EXISTS sales.customer_addresses CASCADE;
DROP TABLE IF EXISTS sales.customers CASCADE;
DROP TABLE IF EXISTS sales.customer_groups CASCADE;
DROP TABLE IF EXISTS sales.price_tiers CASCADE;
DROP TABLE IF EXISTS sales.discount_events CASCADE;
DROP TABLE IF EXISTS sales.discount_event_products CASCADE;
DROP TABLE IF EXISTS sales.discount_event_categories CASCADE;

DROP TABLE IF EXISTS purchase.receipt_items CASCADE;
DROP TABLE IF EXISTS purchase.receipts CASCADE;
DROP TABLE IF EXISTS purchase.order_items CASCADE;
DROP TABLE IF EXISTS purchase.orders CASCADE;
DROP TABLE IF EXISTS purchase.supplier_prices CASCADE;
DROP TABLE IF EXISTS purchase.suppliers CASCADE;

DROP TABLE IF EXISTS master_data.product_images CASCADE;
DROP TABLE IF EXISTS master_data.product_variants CASCADE;
DROP TABLE IF EXISTS master_data.products CASCADE;
DROP TABLE IF EXISTS master_data.attributes CASCADE;
DROP TABLE IF EXISTS master_data.categories CASCADE;
DROP TABLE IF EXISTS master_data.units CASCADE;
...
DROP FUNCTION IF EXISTS public.current_user_company() CASCADE;

-- Drop types
DROP TYPE IF EXISTS inventory.movement_type CASCADE;
DROP TYPE IF EXISTS inventory.movement_reason CASCADE;
DROP TYPE IF EXISTS sales.order_status CASCADE;
DROP TYPE IF EXISTS sales.payment_status CASCADE;
DROP TYPE IF EXISTS purchase.po_status CASCADE;
DROP TYPE IF EXISTS public.location_type CASCADE;

-- Drop schemas (cascade handles everything inside)
...
CREATE TABLE public.locations (
    ...
    is_sales_location BOOLEAN DEFAULT false,
    is_purchase_location BOOLEAN DEFAULT true,
    settings JSONB DEFAULT '{}',
    is_active BOOLEAN DEFAULT true NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
    updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE inventory.stocks (
    ...
    quantity_incoming DECIMAL(18, 4) DEFAULT 0,
    quantity_outgoing DECIMAL(18, 4) DEFAULT 0,
...
SELECT '✅ Setup complete! Database ready for NEO WMS.' AS status;

SELECT 
    'Total schemas: ' || COUNT(DISTINCT table_schema) AS schemas,
    'Total tables: ' || COUNT(*) AS tables
FROM information_schema.tables 
WHERE table_schema IN ('public', 'master_data', 'inventory', 'sales', 'purchase', 'accounting')
